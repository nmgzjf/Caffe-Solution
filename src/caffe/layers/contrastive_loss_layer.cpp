...
//        Dtype dist = std::max(margin - sqrt(dist_sq_.cpu_data()[i]), 0.0);//Modified by Jianfeng
		Dtype dist = std::max(margin - sqrt(dist_sq_.cpu_data()[i]), Dtype(0.0)/*0.0*/);//Modified by Jianfeng
        loss += dist*dist;
      }
    }
  }
